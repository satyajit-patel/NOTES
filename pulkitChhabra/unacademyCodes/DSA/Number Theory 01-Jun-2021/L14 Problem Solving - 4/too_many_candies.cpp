// https://www.codechef.com/problems/ICL1903
...
pii intersection(pii r1, pii r2)
{
	int st = max(r1.ff, r2.ff);
	int en = min(r1.ss, r2.ss);

	return (st <= en) ? mp(st, en) : mp(0ll, -1ll);
}
...
int32_t main()
{
	...
	w(tc)
	{
		int n, m, k, x1 = 1, x2 = inf, y1 = 0, y2 = inf;
		cin >> n >> m >> k;
		...
		pii one = find_a_solution(m, k, n);

		pii r1 = get_range(one.ff, g, k, x1, x2);
		pii r2 = get_range(one.ss, g, -m, y1, y2);

		pii r = intersection(r1, r2);
