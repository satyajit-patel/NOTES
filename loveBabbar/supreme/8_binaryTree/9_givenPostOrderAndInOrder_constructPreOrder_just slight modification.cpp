...
Node* builtdTreeFromPreOrderAndInOrder(vi& postOrder, std::unordered_map<int, int>& inOrderMap, int& postOrderEndIndex, int inOrderStartIndex, int inOrderEndIndex) {
  // base case
  ...
  // recursive case
  root->right = builtdTreeFromPreOrderAndInOrder(postOrder, inOrderMap, postOrderEndIndex, midIndex+1, inOrderEndIndex); // in postOrder first make the call for right subtree. This is only the edge case 
  root->left = builtdTreeFromPreOrderAndInOrder(postOrder, inOrderMap, postOrderEndIndex,inOrderStartIndex, midIndex-1);
  return root;
}
...
void input() {
  ...
  Node* root = builtdTreeFromPreOrderAndInOrder(postOrder, inOrderMap, postOrderEndIndex, inOrderStartIndex, inOrderEndIndex);
